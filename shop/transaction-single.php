...
<main>
  ...
  <div class="transaction-single">
    <div class="transaction-single__header">
      <!-- INFORMASI TRX -->
      <h1>
        Транзакции #FF<?= $order['order_id'] ?> <span class="badge <?= $order['order_status'] == 'paid' ? 'badge_success' : 'badge_danger' ?>"><?= ucwords($order['order_status']) ?></span>
      </h1>
    ...
    <div class="transaction-single__body">
      ...
      <div class="transaction-single__method">
        <div class="transaction-single__method-group">
          <h2 class="transaction-single__method-title">Способы оплаты</h2>
          ...
          <?php if ($order['order_status'] == 'unpaid') : ?>
            <div>
              ...
              <a href="./transaction-edit-payment-method.php?order_id=<?= $order['order_id'] ?>">Изменить способ оплаты</a>
            </div>
          <?php endif; ?>
        </div>
        <!-- TUTOR -->
