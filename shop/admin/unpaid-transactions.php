...
<div class="admin">
  <div class="admin__header">
    <h1 class="admin__title">Неоплаченные транзакции</h1>
  </div>
        ...
        <tbody>
          <?php foreach ($order_info as $order) : ?>
            <tr>
              ...
              <td>Банк <?= $order["payment_method_bank"] ?></td>
              <td><?= date('d M Y', strtotime($order["order_date"])) ?></td>
              <td><?= get_order_sum($order["order_id"]) ?></td>
              <td>₽<?= number_format($order["order_total_price"]) ?></td>
              <td>
                <a href="./unpaid-transaction-approved.php?order_id=<?= $order["order_id"] ?>" class="button">Отметить, как оплаченный</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
