...
<div class="admin">
  <div class="admin__header">
    <div class="admin__back">
      ...
      <a href="./paid-transactions.php">Назад</a>
    </div>
    <h1 class="admin__title">Детали транзакции оплачены</h1>
  </div>
          ...
          for ($don = 0; $don <= 4; $don++) {
            if ($rait == 0) {
              ...
              if ($don == 0) {
                ...
                echo "<td>₽" . number_format($totalprice) . "</td></tfoot>";
              } else {
                echo '<td> </td>';
              }
              ...
              echo '<td>₽' . number_format($plantpricearray[$rait - 1]) . '</td>';
              ...
              if ($rait != $len) { // kolom 4 untuk sub-total
                echo '<td>₽' . number_format($plantpricearray[$rait - 1] * $plantquantityarray[$rait - 1]) . '</td>';
                $totalprice = $totalprice + $plantpricearray[$rait - 1] * $plantquantityarray[$rait - 1];
              } else if ($rait == $len) {
                echo '<td>₽' . number_format($plantpricearray[$rait - 1] * $plantquantityarray[$rait - 1]) . '</td><tbody>';
                $totalprice = $totalprice + $plantpricearray[$rait - 1] * $plantquantityarray[$rait - 1];
              } else {
                echo '<td> </td>';
              }
