...
<div class="admin">
  <div class="admin__header">
    <h1 class="admin__title">Транзакция не была оплачена</h1>
  </div>
  <div class="admin__body">
    ...
    <form action="./manager-unpaid-transactions.php" method="get" class="admin__filters">
      ...
      <span>до тех пор</span>
      <input type="date" class="input" name="dateend" value="<?= $dateend ?>">
      <button class="admin__button">Фильтр</button>
      <a href="./manager-unpaid-transactions.php" class="admin__button">Сброс</a>
    </form>
    <div class="admin__card">
      <h2 class="admin__card-title">График</h2>
      <hr />
      <!-- grafik chart -->
        ...
        for ($rait = 0; $rait <= $drop; $rait++) { //$don = kolom tabel, $rait = baris tabel
          for ($don = 0; $don <= 5; $don++) {
            if ($rait == 0) { //header tabel
              if ($don == 0) {
                echo "<thead><tr><th>N</th>";
              } else if ($don == 1) {
                echo '<th>Способы оплаты</th>';
              } else if ($don == 2) {
                echo '<th>Дата</th>';
              } else if ($don == 3) {
                echo '<th>Цена</th>';
              ...
              echo '<td>Банк ' . $paymentmethodarray[$rait - 1]['payment_method_bank'] . '</td>';
              ...
              if ($rait != $drop) { // kolom 4 untuk harga 
                echo '<td>₽' . number_format($orderpricearray[$rait - 1]['order_total_price']) . '</td>';
